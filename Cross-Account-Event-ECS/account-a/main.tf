...
resource "aws_iam_policy" "forward_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:events:us-east-1:496390993498:event-bus/ec2-events-bus"
    }]
  })
}
...
resource "aws_cloudwatch_event_target" "send_to_a_bus" {
  rule      = aws_cloudwatch_event_rule.ec2_all_events.name
  arn       = "arn:aws:events:us-east-1:496390993498:event-bus/ec2-events-bus"
  role_arn  = aws_iam_role.eventbridge_forwarder.arn
  target_id = "send-to-B-custom-bus"
}
